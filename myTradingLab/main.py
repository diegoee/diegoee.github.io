...
class Acount:
  ...
  def __init__(self,cast=None):
    ...
    if (cast is not None):
      self.cast=cast
    print("Acount created. Cast=",self.cast) 
  ...
  def printStatusAcount(self):
    ...
    print(' Real Cast: ',self.realCast,' (used:',round((self.margin*1.00/self.realCast)*100,2),'%)') 
  ...
  def updateAcount(self):
    ...
    self.benefitLoss = self.positions['B/L'].sum()
    self.margin      = self.positions['Margin'].sum()
    self.available   = self.cast - self.margin + self.benefitLoss
...
class MyTradingStrategy:
  ...
  def method_EURUSD_01(self):
    ...
    pos03 = acount.createPosition(margin=333)
